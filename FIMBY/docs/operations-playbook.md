...
| "Your posts, stories, library listings, and group-chat messages are hard-deleted within 24 hours of the nightly retention batch" | `FimbyAccountDeactivationBatch.deleteOwnedContent` covers `Notification__c`, `Story__c`, `Needs_Offers__c`, `Library_Item__c`, `Shared_Contact_Info__c`, `Conversation_Member__c`, `Message__c`; support relationships are handled separately by `cleanUpSupportRelationships` | Runs nightly per the schedule in runbook §2 |
| "Where you are the last approved guardian, the young person's profile is deleted along with your account, on the same nightly cleanup" | `cascadeManagedProfiles` captures candidate children before removing mandates, then `FimbyManagedProfileCascadeService.resolveAfterGuardianRemoval` flags orphans and merges unheld ones into the same `execute()` | Fail-closed: a cascade failure rolls the whole chunk back. See [runbook — parent-managed profiles](./runbooks/account-deletion.md) |
| "Where another parent or guardian is still approved, the profile carries on unchanged and they keep looking after it" | The same resolve pass repoints `Contact.Proxied_By_Contact__c` to the oldest surviving mandate and sends **no** notification | Silent on purpose — a notice would disclose that another adult deleted their account |
| "We tell you that on screen before you confirm, so it is never a surprise" | `FimbyProfileController.getManagedProfilesAtRisk` drives a named warning in the `fimbySettingsView` delete modal; `FimbyDeletionEmailService.sendVerification` carries the same warning on the email-to-Case path, which has no screen | Warning is deliberately pessimistic about co-guardians already in grace |
...
1. `FimbyAccountDeactivationBatch.cleanUpSupportRelationships` deletes every `Support_Relationship__c` where the deleted Contact is an actual **party** (`Contact__c IN :contactIds OR Related_Contact__c IN :contactIds`). Rows where they only appear in `Approved_By__c` / `Ended_By__c` belong to other people and are **kept** with those audit lookups nulled — deleting them used to revoke a surviving co-guardian's mandate and orphan a child profile.
...
**Parent-managed profiles do not wait for the batch.** When a guardian removes a child profile through Manage Identities, the child Contact is flagged `Deactivation_Requested__c` but stays alive until the nightly scrub. `FimbySupportRelationshipController` filters or refuses on that flag in `getAvailableIdentities`, `getManagedRelationships`, `switchIdentity`, and `IdentityReader.approvedMandateFor`, so the profile leaves the switcher and the Manage Identities list immediately — which is what the notice sent to the other guardians already claims. If community-group identities ever disappear from someone's switcher, check that those filters are still null-tolerant (`Related_Contact__c = NULL OR ...`): organization-backed identities have no `Related_Contact__c`.
...
- [ ] **Account deactivation batch errors reviewed.** Search `Error_Log__c` for `Source = 'FimbyAccountDeactivationBatch'` in the last 30 days. Investigate any. **A repeating signature here matters more than a one-off:** the parent-managed cascade is fail-closed, so a deterministic bug rolls the same chunk back every night while Apex Jobs still looks clean and the adults stay flagged. Do not read quiet job history as success.
