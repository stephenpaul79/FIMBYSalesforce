...
## 6. Push deep-link metadata
`FimbyPushNotificationService.PushPayload.data` carries optional `action_url` (= `Notification__c.Action_URL__c`) and `notification_id` on single-notification pushes; batched pushes include `action_url` only when the batch covers exactly one notification. Mobile `resolvePushRoute()` routes by `action_url` first (deep-link to the exact record, host-validated via `parseFimbyDeepLink`), then falls back to `channelId` (→ `/messages` for `messages*`, else `/notifications`). Single-notification pushes carry the real `Title__c`/`Body__c`. Multi-item pushes carry a **name-led aggregate** built in `FimbyPushBatchJob` (`buildAggregateTitle`/`buildAggregateBody`): lead with one real `Actor_Name__c`, fall back to `'A neighbour'` (then to today's generic strings when no name exists), team/icon tokens (`'FIMBY'`, `'Neighbourhood team'`, `'care'`, `'system'`) skipped, 80/140 caps truncated on a word boundary, body never `null`. `action_url`/`notification_id` ride only on single-notification pushes.

**Two gates never merge into one ping.** A mixed DM+activity cycle emits up to **two** device pings — one DM (`collapseId` `{userId}:dm`, channel `messages`/`messages_quiet`) and one activity (`{userId}:activity`, channel `activity`) — so each routes to the surface where its content lives. Collapsing happens only *within* a gate (a new cycle replaces that gate's card in place). There is no `:both` ping. (The `BOTH` gate-stamp label is unrelated: it stamps both doorbell timestamps for one user.)
